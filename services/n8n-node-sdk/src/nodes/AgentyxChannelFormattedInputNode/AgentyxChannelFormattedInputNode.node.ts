...
export class AgentyxChannelFormattedInputNode implements INodeType {
  description: INodeTypeDescription = {
    ...
    description:
      "Normalize channel-specific payloads (Kapso WhatsApp, Telegram, Meta, LibreChat, Twenty Webhook) into a canonical envelope",
    defaults: {
    ...
    properties: [
      ...
      },
      {
        ...
        default: 15,
        description:
          "Time window to detect duplicate message_ids. 0 = disabled. Uses $getWorkflowStaticData('global').",
  ...
  async execute(
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const coreWorkflowId = this.getNodeParameter("coreWorkflowId", i) as string;
      const dedupTtl = this.getNodeParameter("dedupTtlMinutes", i, 0) as number;
      ...
      switch (channel) {
        case "kapso-wa": {
          ...
          if (msg.type === "text" && msg.text) {
            const txt = msg.text as Record<string, string>;
            ...
            const voice = msg.voice as Record<string, string>;
        ...
        case "meta-comment": {
          ...
          const commentText = String(
            payload.message || payload.comment_text || payload.text || "",
          );
          ...
          const senderName = String(
            payload.sender_name || (payload.from as Record<string, string>)?.name || "Usuario",
          );
          const senderId = String(
            payload.sender_id || (payload.from as Record<string, string>)?.id || "",
          );
        ...
        case "twenty-webhook": {
          ...
          const body = (input.body as Record<string, unknown>) || input;
      ...
      if (dedupTtl > 0 && envelope.metadata && (envelope.metadata as Record<string, unknown>).message_id) {
        try {
          // n8n static data access
          const messageId = String((envelope.metadata as Record<string, unknown>).message_id || "");
          ...
          const seen = (staticData.seenMessageIds as Record<string, number>) || {};
          const now = Date.now();
